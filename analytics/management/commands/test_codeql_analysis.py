...
class Command(BaseCommand):
    ...
    def _show_summary(self, repo_name):
        ...
        # Calculate and show security level (uses existing API)
        from analytics.codeql_service import CodeQLService
        codeql_service = CodeQLService()
        level_metrics = codeql_service.calculate_security_level(vulnerabilities)
        level_display = level_metrics.get('level_display') or level_metrics.get('level', 'unknown').title()
        self.stdout.write(f'\nSecurity Level: {level_display}')
